...
class WVM:
    ...
    def ILOAD_LOCAL(self, slot):
        assert isinstance(slot, int), slot        
        self.IPUSH(self.frames[-1][slot])

    def FLOAD_LOCAL(self, slot):
        assert isinstance(slot, int), slot        
        self.FPUSH(self.frames[-1][slot])

    def ISTORE_LOCAL(self, slot):
        assert isinstance(slot, int), slot        
        self.frames[-1][slot] = self.IPOP()

    def FSTORE_LOCAL(self, slot):
        assert isinstance(slot, int), slot        
        self.frames[-1][slot] = self.FPOP()
    ...
    def CALL(self, name):
        self.frames.append({})
        self.frames[-1]['return'] = self.pc
        self.pc = self.labels[name]

    def RET(self):
        self.pc = self.frames[-1]['return']
        self.frames.pop()
...
def generate(node, context):
    if isinstance(node, Integer):
        ...
        valtype = generate(node.value, context)
        if valtype == 'int':
