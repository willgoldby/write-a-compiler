...
# Top-level function used to check a program.  Program is an object that
# holds information about the program including the model.
def check_program(program):
    context = CheckContext(program)
    check_node(program.model, context)
...
def check_node(node, context):
    ...
    if isinstance(node, Integer):
        ...
        left_type = check_node(node.left, context)
        right_type = check_node(node.right, context)
        ...
        valuetype = check_node(node.value, context)
